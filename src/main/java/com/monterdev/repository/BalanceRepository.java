...
public interface BalanceRepository extends CrudRepository<Balance,Integer> {
    ...
    @Query(value = "SELECT sum(a.endbalance) FROM  balance a where year=:year and month=:month and category in (SELECT a.category_name FROM itemcategory a);", nativeQuery = true)
    double getTotalBeginningBalanceForThisMonth( @Param("month") String month, @Param("year") String year);

    @Query(value = "SELECT * FROM  balance a where year=:year and month=:month and category in (SELECT a.category_name FROM itemcategory a);", nativeQuery = true)
